...
<style>
/* ===== ESTILOS COMPLETOS (incluir desde el archivo anterior por brevedad) ===== */
/* Por favor, copia aquí todo el CSS del mensaje anterior o mantenlo como ya lo tienes */
:root{
...
.extra-card{background:var(--bg2);border:1px solid var(--border);border-radius:var(--r);padding:16px 18px;box-shadow:var(--sh)}
.tec-item{display:flex;align-items:center;gap:10px;padding:8px 10px;background:var(--bg3);border-radius:8px;border:1px solid var(--border);margin-bottom:8px}
.tec-avatar{width:32px;height:32px;border-radius:50%;background:var(--accent-l);display:flex;align-items:center;justify-content:center;font-size:13px;flex-shrink:0}
.tec-info{flex:1;min-width:0}
.tec-name{font-size:12px;font-weight:600;color:var(--text);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.tec-bar{height:4px;background:var(--border);border-radius:2px;margin-top:4px}
.tec-bar-fill{height:4px;border-radius:2px;transition:.3s}
.tec-stats{display:flex;gap:6px;flex-shrink:0;align-items:center}
.tec-pend{font-family:'JetBrains Mono',monospace;font-size:13px;font-weight:700;color:var(--danger)}
.tec-urg{border-radius:20px;padding:2px 6px;font-size:11px;font-weight:600;background:var(--urgent-l);color:var(--urgent)}
.tec-done{font-family:'JetBrains Mono',monospace;font-size:11px;color:var(--success)}
.desc-item{padding:6px 8px;background:var(--bg3);border-radius:6px;border:1px solid var(--border);margin-bottom:5px}
.desc-text{font-size:11px;color:var(--text2);white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.desc-meta{font-size:10px;color:var(--text3);margin-top:2px;display:flex;gap:6px}
...
.pagination{display:flex;gap:6px;justify-content:center;margin-top:24px;margin-bottom:16px}
.page{padding:6px 12px;background:var(--bg2);border:1px solid var(--border);border-radius:6px;font-size:13px;color:var(--text2);text-decoration:none;transition:.12s}
.page.active{background:var(--accent);color:#fff;border-color:var(--accent)}
.page:hover:not(.active){border-color:var(--accent);color:var(--accent)}
@media(max-width:960px){.layout{grid-template-columns:1fr}.sidebar{display:none}.stats-row{grid-template-columns:repeat(3,1fr)}.mid{grid-template-columns:1fr}.rg{grid-template-columns:repeat(4,1fr)}.extra-grid{grid-template-columns:1fr}}
...
<aside class="sidebar">
    ...
    <div class="sb-sec">
        ...
        ?>
        <a href="?f_search=<?php echo urlencode($resid);?>" class="sb-item <?php if($f_search===$resid) echo 'on';?>" style="font-size:12px">
...
<div class="mid">
    ...
    <div class="panel">
        ...
        <div class="rg">
            ...
            ?>
            <div class="rc">
                <div class="rn"><?php echo $resid;?></div>
...
<!-- Paneles extra: carga de técnicos + basurero -->
<div class="extra-grid">
    ...
    <div class="extra-card">
        ...
        <div>
            ...
            ?>
            <div class="tec-item">
                <div class="tec-avatar">👷</div>
                <div class="tec-info">
                    <div class="tec-name"><?php echo htmlspecialchars($tec['tecnico_asignado']);?></div>
                    <div class="tec-bar"><div class="tec-bar-fill" style="width:<?php echo $pct;?>%;background:<?php echo $color;?>"></div></div>
                </div>
                <div class="tec-stats">
                    <span class="tec-pend"><?php echo $tec['pendientes'];?></span>
                    <?php if($tec['urgentes'] > 0): ?>
                    <span class="tec-urg">🚨<?php echo $tec['urgentes'];?></span>
                    <?php endif;?>
                    <span class="tec-done">✓<?php echo $tec['terminados'];?></span>
                </div>
            </div>
            <?php endforeach;?>
        </div>
    ...
    <?php else: ?>
    <div class="extra-card" style="display:flex;align-items:center;justify-content:center;color:var(--text3);flex-direction:column;gap:8px">
    ...
    <div class="extra-card">
        ...
        <div style="border-top:1px solid var(--border);padding-top:10px;margin-top:4px">
            ...
            <div class="desc-item">
                <div class="desc-text"><?php echo htmlspecialchars($d['mensaje'] ?? '');?></div>
                <div class="desc-meta">
                    <span><?php echo htmlspecialchars($d['remitente']??'');?></span>
                    <span class="desc-badge"><?php echo $d['motivo'];?></span>
                </div>
            </div>
            <?php endforeach;?>
        </div>
...
<div class="toolbar">
    <div class="trow">
        ...
        <input type="date" name="f_fecha_fin" class="fsel" value="<?php echo $f_fecha_fin;?>" title="Hasta" onchange="document.getElementById('ff').submit()">
...
<?php
if ($res === false) {
    echo '<div class="empty"><div class="empty-ico">⚠️</div>
          <strong>Error en la consulta:</strong> ' . htmlspecialchars($conn->error) . '<br>
          <small>SQL: ' . htmlspecialchars($sql) . '</small></div>';
} elseif (is_string($res)) {
    echo '<div class="empty"><div class="empty-ico">⚠️</div>
          <strong>Error inesperado:</strong> $res es un string con valor: ' . htmlspecialchars($res) . '<br>
          Esto ocurre porque en algún bucle se usó $res como variable y pisó el resultado de la consulta.<br>
          <strong>Ya fue corregido renombrando las variables en los bucles de residenciales.</strong><br>
          <small>SQL que se intentó ejecutar: ' . htmlspecialchars($sql) . '</small></div>';
} elseif ($res->num_rows === 0) {
    echo '<div class="empty"><div class="empty-ico">📭</div>
          <p>Sin resultados con estos filtros</p></div>';
} else {
