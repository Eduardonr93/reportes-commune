...
// Para activar login: agregar sesión PHP y asignar $rol_activo según usuario
$roles = [
...
$rol_activo = 'admin'; // ← cambiar cuando se active login
...
/* Topbar */
.topbar{background:var(--bg2);border-bottom:1px solid var(--border);padding:0 20px;height:58px;display:flex;align-items:center;gap:12px;position:sticky;top:0;z-index:100;box-shadow:var(--sh)}
...
@media(max-width:580px){     .stats-row{grid-template-columns:repeat(2,1fr)}     .stats-row .sc:last-child{grid-column: span 2}.main{padding:12px}.topbar{padding:0 12px}.rg{grid-template-columns:repeat(2,1fr)}}
</style>
</head>
<body>
...
elseif($res->num_rows===0){ echo '<div class="empty"><div class="empty-ico">📭</div><p>Sin resultados con estos filtros</p></div>'; }
    ...
    if($row['fecha']){
        ...
        if($row['estatus']==='Terminado'){ $sla="✓ Resuelto en {$h}h"; $sc='sla-d'; }
        else $sla="Abierto hace {$h}h";
    }
?>
<div class="card <?php echo $urg?'urg':'';?>" id="c<?php echo $id;?>">
    ...
    <?php endif;?>
    <?php if($sla): ?>
    <div class="csla"><span class="<?php echo $sc;?>"><?php echo $sla;?></span></div>
    <?php endif;?>
    <?php if(can('edit')): ?>
    <div class="cf">
        <form method="POST">
...
// Alerta reporte terminado (cookie post-redirect)
